      subroutine res_mg_psi_t0(R,Z,n_f,n_psi_t0,n_psi_t0_rhs,NZ,NR,hR,hZ
     &,myzero,phys_bdy,cmask,res)
      ...
      qb = n_psi_t0(i, j) - 0.1D1 * n_psi_t0_rhs(i, j) - 0.1D1 * myzero 
     #* Z(i) * R(j)
      res(i,j)=qb
      endif
      end do
...
c======================================================================
     ...
     #_psi_t0(i + 1, j)) / hZ ** 2 - 0.1D1 * n_f(i, j) ** 2 - 0.1D1 * n_
     #psi_t0_rhs(i, j)
      res(i,j)=qb
      endif
      end do
...
c      write(*,*) 'residuals:'
c      do i=1, NZ,1
c      write(*,"(30e15.5)") ( res(i,j), j=1,NR )
c      end do
c      endif
