...
c-----------------------------------------------------------------------
        subroutine lop(LV,V,phi_r,phi_i,cmask,x,y,Nx,Ny)
        implicit none
        integer Nx,Ny
        real*8 V(Nx,Ny),phi_r(Nx,Ny),phi_i(Nx,Ny)
        real*8 cmask(Nx,Ny),LV(Nx,Ny)
        real*8 x(Nx),y(Ny)
        ...
        do i=2,Nx-1
         do j=2,Ny-1
          if (cmask(i,j).eq.CMASK_ON) then
           LV(i,j) = (V(i+1,j)-2*V(i,j)+V(i-1,j))/dx2
     &             + (V(i,j+1)-2*V(i,j)+V(i,j-1))/dy2
     &             - 0.5*(phi_r(i,j)**2+phi_i(i,j)**2)
          end if
         end do
        end do
...
c-----------------------------------------------------------------------
        subroutine residual(res,rhs,V,phi_r,phi_i,cmask,x,y,norm,
     &                      Nx,Ny)
        implicit none
        integer Nx,Ny
        real*8 V(Nx,Ny),phi_r(Nx,Ny),phi_i(Nx,Ny)
        real*8 cmask(Nx,Ny),res(Nx,Ny),rhs(Nx,Ny)
        real*8 x(Nx),y(Ny),norm

        integer i,j,sum

        include 'cmask.inc'

        call  lop(res,V,phi_r,phi_i,cmask,x,y,Nx,Ny)
...
c-----------------------------------------------------------------------
        subroutine relax(V,rhs,phi_r,phi_i,cmask,phys_bdy,x,y,norm,
     &                   Nx,Ny)
        implicit none
        integer Nx,Ny
        real*8 V(Nx,Ny),phi_r(Nx,Ny),phi_i(Nx,Ny)
        real*8 cmask(Nx,Ny),rhs(Nx,Ny)
        real*8 x(Nx),y(Ny),norm
        integer phys_bdy(6)

        integer i,j,N,pass,red,sum
        real*8 PI
        real*8 dx,dy,dx2,dy2
        real*8 v_x,v_y
        ...
        do pass=0,1
         do i=2,Nx-1
          do j=2,Ny-1
           if (mod(i+j+pass,2).eq.0.and.
     &         (cmask(i,j).eq.CMASK_ON)) then
            res=(V(i+1,j)-2*V(i,j)+V(i-1,j))/dx2
     &        + (V(i,j+1)-2*V(i,j)+V(i,j-1))/dy2
     &        - 0.5*(phi_r(i,j)**2+phi_i(i,j)**2)
     &        - rhs(i,j)
...
c        if (Nx .lt. 8 .and. Ny .lt. 8) then
c        write(*,*)
c        write(*,*) 'relax:'
c        write(*,*) phys_bdy(1), phys_bdy(2), phys_bdy(3), phys_bdy(4)
c        do i=1, Nx, 1
c        write(*,"(30F12.5)") ( cmask(i,j), j=1,Nx )
c        end do
c        endif
